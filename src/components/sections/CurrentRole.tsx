import { currentRole } from "@/content/data";
              ...
              <div className="relative">
                <div className="flex flex-wrap items-center gap-3 font-mono text-xs">
                  <span className="inline-flex items-center gap-2 rounded-full bg-emerald-400/10 px-3 py-1 text-emerald-300">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /> EM ATIVIDADE
                  </span>
                  <span className="text-muted">{currentRole.period}</span>
                </div>

                <p className="mt-10 font-mono text-sm tracking-[0.3em] text-cyan">MWM</p>
                <h3 className="mt-2 text-3xl font-semibold tracking-tight md:text-5xl">{currentRole.title}</h3>
                <p className="mt-3 text-muted">
                  {currentRole.company} · {currentRole.mode}
                </p>
